module.exports = {
    ...
    async run (bot, message, args) {
       ...
       if(!bot.cola){
        ...
        if(!volumen){
            return message.channel.send('<:not:786381128260911155> | Agrega el volumen')
        }
        let numeroVolumen = parseInt(volumen);

        if(!Number.isInteger(Number(volumen))) {
            return message.channel.send('<:not:786381128260911155> | Solo numeros, no letras')
        }

  if(numeroVolumen < 1) {
    return message.channel.send('<:not:786381128260911155> | El volumen no puede ser menor que 1')
  }

 if(numeroVolumen > 100){
 return message.channel.send('<:not:786381128260911155> | El volumen no puede ser mayor que 100')
 }
