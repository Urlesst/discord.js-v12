...
module.exports = {
    ...
    async run(bot, message, args) {
      ...
      let usuariosPermitidosAusarElEval = ['777903463828815894', '753381968418308167', '285854417004134401']
      if(!usuariosPermitidosAusarElEval.includes(message.author.id)) return;

       try {
        const salidaDeLoEvaluado = await eval(contenidoAEvaluar || undefined) 
        message.channel.send(`${await Date.now() - message.createdTimestamp}ms\n(${typeof(salidaDeLoEvaluado) || 'Nada que mostrar'}) ${inspect(salidaDeLoEvaluado, { depth: 0}).replace(bot.token, 'No dejare que veas esto')}`, {split:  {char: '', maxLength: 1950}, code: 'js'})
          ...
          return bot.users.cache.get(usuariosPermitidosAusarElEval[0]).send(`@ ${message.author.tag} Evaluo\n # ${contenidoAEvaluar}`, {split:  {char: '', maxLength: 1950}, code: 'py'})
          }
       } catch (err) {
